...
void HostDbObject::OnConfigUpdate(void)
{
	...
	BOOST_FOREACH(const Checkable::Ptr& checkable, host->GetParents()) {
		...
		Log(LogDebug, "HostDbObject", "host parents: " + parent->GetName(), checkable->IsLogVerbose() || parent->IsLogVerbose());
	...
	/* host dependencies */
	Log(LogDebug, "HostDbObject", "host dependencies for '" + host->GetName() + "'", host->IsLogVerbose());

	BOOST_FOREACH(const Dependency::Ptr& dep, host->GetDependencies()) {
		Checkable::Ptr parent = dep->GetParent();

		if (!parent) {
			Log(LogDebug, "HostDbObject", "Missing parent for dependency '" + dep->GetName() + "'.", host->IsLogVerbose() || dep->IsLogVerbose());
			continue;
		}

		int state_filter = dep->GetStateFilter();

		Log(LogDebug, "HostDbObject", "parent host: " + parent->GetName(), host->IsLogVerbose() || dep->IsLogVerbose() || parent->IsLogVerbose());
	...
	Log(LogDebug, "HostDbObject", "host contacts: " + host->GetName(), host->IsLogVerbose());

	BOOST_FOREACH(const User::Ptr& user, CompatUtility::GetCheckableNotificationUsers(host)) {
		Log(LogDebug, "HostDbObject", "host contacts: " + user->GetName(), host->IsLogVerbose() || user->IsLogVerbose());
	...
	Log(LogDebug, "HostDbObject", "host contactgroups: " + host->GetName(), host->IsLogVerbose());

	BOOST_FOREACH(const UserGroup::Ptr& usergroup, CompatUtility::GetCheckableNotificationUserGroups(host)) {
		Log(LogDebug, "HostDbObject", "host contactgroups: " + usergroup->GetName(), host->IsLogVerbose() || usergroup->IsLogVerbose());
